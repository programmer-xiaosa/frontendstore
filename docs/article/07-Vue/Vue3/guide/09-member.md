...
- `jsx` [语法，需要简单了解其基本使用](https://v3.cn.vuejs.org/guide/render-function.html#dom-%E6%A0%91)
...
export default {
  ...
  render () {
    // 返回的内容会进行渲染
    // 1. 在 babel 的帮助下，可以在vue中写JSX语法

    // 数据的使用
    const title = 'Hello JSX!'

    // 事件如何绑定 
    // **JSX语法当中不能写任何Vue指令，因为渲染函数里面写的根本不是Vue的模板**
    const say = () => {
      console.log('Hello JSX Function')
    }

    // 定义一个JSX对象(如果有多个标签可以用数组)
    const sub = [<sub>Hello sub title1!</sub>, <sub>Hello sub title2  !</sub>]

    return <h2 title={ title } onClick={ say } > { title } { sub }</h2>
  }
}
</script>
```
