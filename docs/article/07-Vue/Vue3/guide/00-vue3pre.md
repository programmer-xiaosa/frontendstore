...
高级用法：**默认情况下computed计算属性的值不能修改，只读模式，如果需要修改要用计算属性的高级用法，了解即可，不建议修改computed值**
...
> 如果上面的课程还是不理解的话请看下面链接应该会好一点
- [vue3 如何通过 ref 获取 DOM 节点](https://www.jianshu.com/p/2009f3beb7a5)
- [vue3如何通过ref属性获取元素](https://www.jianshu.com/p/db72d66dad0a)
...
::: warning 解释
{emit} 这里通过解构的方式去引入emit方法，实际setup的第二参数是一个context上下文，通过解构的方式在context中拿到emit方法
:::
...
> 默认情况下后代组件不能修改 `provide` 提供的数据【遵守单向数据流概念】，如果要修改的话必须通过让provide提供父组件的函数来更新数据，请参考以上案例。
...
在vue2.0中 `v-model` 语法糖简写的代码：
```vue
<Son :value="msg"  @input="msg=$event" />
``` 
**解释上面代码：**
```vue
   // v-model 其实在vue中相当于两个事件（也可以这么说）
   <input v-model="value">
   //在组件中等价于
   <p-input :value="value" @input="value=$event.target.value"></p-input> 
   //这里的input事件并不是input的监听输入的事件，而是自己自定义组件中发布的事件，$event是自己自定义组件中发布的事件
```

在vue3.0中 `v-model` 语法糖有所调整：
```vue
...
:::tip 补充
+ 如果父传子 `v-model:visible` 子传父 `@update:visible`
