...
> 在Home目录下->components目录中创建一个home-panel组件
...
**总结：** vue3.0中  只支持v-slot指令，所以需要配合template来使用。v-slot也有缩写，即把参数之前的所有内容：v:slot替换为字符 `#`。例如 `v-slot:header` 可以被重写为#header。 
...
多个transition使用不同动画，可以添加 `name` 属性， `name` 属性的值替换v即可。



## 23-首页主体-面板骨架效果

::: danger TODO: 需要抽时间单独做一下
**目的：** 加上面板的骨架加载效果
:::
...
- 在img上使用v-lazyload值为图片地址，不设置src属性。
