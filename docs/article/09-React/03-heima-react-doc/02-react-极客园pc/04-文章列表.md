...
+ 可以在layout组件的componentDidUpdate中进行修改，只要路由发生变化就会触发
  ...
  ![image-20211031203845128](images/image-20211031203845128.png)

+ AuthRoute组件将layout组件通过render交给了Route组件

  ![image-20211031204059758](images/image-20211031204059758.png)
...
+  [componentDidUpdate](https://reactjs.org/docs/react-component.html#componentdidupdate) 更新defaultSelectedKeys
...
+ 删减修改状态区域
  ...
  [单选框组合](https://ant.design/components/radio-cn/#components-radio-demo-radiogroup-with-name)
...
onOk: async () => {
  ...
  await delArticle(id)
  // console.log(id)
  const { articleList, page } = this.state
  this.setState(
    {
      articleList: articleList.filter(item => item.id !== id),
      page:
      articleList.length === 1 && page !== 1 ? page - 1 : page
    },
    function () {
      this.formRef.current.submit()
    }
...
+ 图片上传功能， 需要提供name和action参数
  ...
  ![image-20211111185951947](images/image-20211111185951947-6628395.png)
...
export const addArticle = (data) => {
  return request({
    url: '/mp/articles?draft=' + data.draft,
    method: 'post',
    data,
  })
}
