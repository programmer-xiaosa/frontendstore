...
- template标签不能加v-show，因为template根本不是一个元素，只是包裹某个元素决定显示隐藏而已
...
+ [Vue中Vnode的创建与处理](https://blog.csdn.net/qq_26443535/article/details/109678854)
+ [VUE3中h()函数和createVNode()函数的使用](https://blog.csdn.net/qq_17355709/article/details/112712786)
...
:::warning 知识扩展
如果想在当前的组件中通过获取当前组件的实例来调用的话请通过这种方式来实现

<!-- 1.先拿到组件实例 -->
const { proxy } = getCurrentInstance() 

<!-- 2.通过实例对象来调用 -->
proxy.$message({ text: '提示框' })
...
+ 	</div>
+  </section>
+  <section class="container" v-else></section>
```
