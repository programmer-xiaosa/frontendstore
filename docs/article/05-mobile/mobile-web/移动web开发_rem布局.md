...
rem (root em)是一个相对单位，类似于em，em是父元素字体大小（em单位先从元素本身找字体大小，如果在以自身为主，如果没有找父元素，一似类推一直往上找）。
...
+ px to rem 插件能把px单位转换为rem单位
> px to rem插件使用的时候先右键看到扩展设置里面有设置默认html大小的地方，然后写完所有单位使用alt+z快捷键可以快速生成rem单位
